...
const Fetcher = ({realizingValue, setRealizingValue, ...props}) => {
	...
	const address = 'tz1PBLXtgPX2cdFkiFcifzKDDWUyQuXdXjbp'
  	const fiat = 'USD'
	...
	const query2 =  async (quantityRealizing) => {
		console.log("here")
		await axios.get(`http://api.portaltoblockchain.org/Analysis/Tezos/AutoRealize
		?address=${address}&fiat=${fiat}&realizingQuantity=${quantityRealizing}`).then((data)=>{
			console.log(data)
				...
				//doc.addImage(tezLogo, 'JPEG', 20, 25, 23, 23, 'Tezos Logo');
					...
					doc.text("TEZOS DELEGATOR ADDRESS: " + data.data.address, 25, 67)
					doc.text("FIAT: " + data.data.fiat, 25, 74)
					//var qRewSold = data.realizingRewardAgg.toFixed(2)
					var last = data.data.realizingRewards.length
					doc.text("PERIOD START: " + data.data.realizingRewards[0]["date"], 25, 88);
					doc.text("PERIOD END: " + data.data.realizingRewards[last - 1]["date"], 25, 95);
					doc.text("QUANTITY OF REWARDS SOLD: " + numberWithCommas(data.data.realizingRewardAgg.toFixed(2)) + " XTZ", 25, 102)
					doc.text("AVERAGE BASIS COST: " + data.data.basisPrice.toFixed(2) + " " + data.data.fiat, 25, 109)
					doc.text("TRUE REWARD INCOME: " + numberWithCommas(data.data.realizingRewardBasisAgg.toFixed(2)) + " " + data.data.fiat, 25, 116)
			//doc.setFontSize(12)
